...
各自釘在哪個版本，並算出離上游有多遠。結果會出現在四個地方：

- **一張關係圖**（預設）：**一個 block ＝ 一個 repo 的一個版本**，同一個 repo 被釘在兩個版本就畫成兩個 block。
  線是曲線，垂直進出、左右位移只發生在層與層之間，不會穿過節點。
...
- **一張版本比較表**（選了「比較」版本才會出現）：一列一個 repo，兩欄放兩個版本各自綁到哪一版
...
同一個 repo 被不同的上層釘在不同 commit 時，圖上會畫成**好幾個 block**，一個版本一個，
每條線各自接到它真正指向的那一版（`caliptra-ss` 通常是三個、`caliptra-dpe` 兩個）。
滑到 block 上會顯示是被哪幾個地方釘成這一版的。
起點留在「全部 repo」時每個 repo 只有一個 block，顯示上游最新版。

同一個 repo 在樹裡被綁到兩個不同版本（例如 `hw/latest` 與 `hw/rev-2_1` 指向不同的 caliptra-ss）
也會一起列出來。
...
| 同上，再加一個「比較」版本 | 再多約 8～12（比較的樹只解版本，不算落後量） |
